...
import base_config
import global_server_price as gsp
...
def load_config(root):
    try:
        ...
        price_json = gsp.get_global_data(league_name)
            ...
            for y_num in range(11):
                ...
                if kb.is_pressed('end'):
                    ...
                    try:
                        ...
                        # 数量判断
                        if '可堆叠通货' in content or '可叠加通货' in content:
                            item_amount = content[
                        ...
                        if always_flag == 0:
                            ...
                            if '物品类别: 可堆叠通货' in content or '物品类别: 可叠加通货' in content:
                                ...
                                elif "崇高石碎片" in content:
                                    item_name = '崇高石碎片'
                                    item_value = all_config["price_json"]['崇高石'] / 20.0
...
# 标签页1内容如下：
# 创建提示文本
label_remind_1 = tk.Label(tab1, text="图金小助手v0.55(国际服版)", font=("Courier", 15))
